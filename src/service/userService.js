...
import {
  ...
  deleteTempUser,
  findTempUser,
} from "../repository/tempUserRepository.js";
import { withErrorHandling } from "../utils/errors/errorHandler.js";
import { createUser, getAllUserRepository } from "../repository/userRepository.js";
import cloudinary from "../config/cloudinaryConfig.js";

const { EMAIL_USER } = config;


export const registerUser = withErrorHandling(async (data, file) => {
  ...
  // ✅ Upload profile picture using FILE PATH (multer disk storage)
  if (file) {
    const uploadResult = await cloudinary.uploader.upload(file.path, {
      folder: "mentorhub/profile",
      resource_type: "image",
    });
...
export const getAllUsersService = withErrorHandling(async () => {
...
export const resendOtp = withErrorHandling(async (email) => {
  ...
  if (!existingTemp) {
    throw new Error("No OTP found for this email. Please register again.");
  }
